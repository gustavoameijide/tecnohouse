...
export const getVentas = async (req, res, next) => {
  //obtener perfiles
  const result = await pool.query("SELECT * FROM ventas WHERE user_id = $1", [
    req.userId,
  ]);
...
export const getVenta = async (req, res) => {
  const result = await pool.query("SELECT * FROM ventas WHERE id = $1", [
    req.params.id,
  ]);
...
export const createVenta = async (req, res, next) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO ventas (detalle, tipo , numero, total, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *",
...
export const actualizarVenta = async (req, res) => {
  ...
  const result = await pool.query(
    "UPDATE ventas SET detalle = $1, tipo = $2 ,numero = $3, total = $4 WHERE id = $5",
...
export const eliminarVenta = async (req, res) => {
  const result = await pool.query("DELETE FROM ventas WHERE id = $1", [
    req.params.id,
  ]);
...
export const getVentaDelMes = async (req, res, next) => {
  ...
  const result = await pool.query(
    "SELECT * FROM ventas WHERE user_id = $1 AND created_at BETWEEN $2 AND $3",
